...
const FranchiseDocumentList = ({route}) => {
  ...
  const {FranchiseDoc} = useSelector(state => state.JHGroupDoc);
  ...
  const filteredResults = useMemo(() => {
    const data = Array.isArray(FranchiseDoc?.data) ? FranchiseDoc.data : [];

    return data.filter(item =>
      item['File Name']?.toLowerCase().includes(searchQuery.toLowerCase()),
    );
  }, [searchQuery, FranchiseDoc]);
  ...
  const openDocumentLink = link => {
    setPdfLink(link);
    navigation.navigate('PdfView', {pdfLink: link});
  };
...
export default FranchiseDocumentList;
